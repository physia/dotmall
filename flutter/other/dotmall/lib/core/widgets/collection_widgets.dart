...
class _CollectionListPanelState<C extends Collection, M extends Model>
  ...
  void initState() {
    ...
    if (widget.autoStartLoad) {
      controller.add(CollectionPanelLoadListEvent());
    }
    super.initState();
  }
            ...
            child: Flex(
              ...
              children: [
                if (style?.showLeading ?? true) ...[
                  this.leadingBuilder?.call() ??
                      Builder(builder: (context) {
                        Widget _child = Container(
                          margin: const EdgeInsets.all(4),
                          decoration: ((semantic == null
                                      ? null
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Column(
                            crossAxisAlignment: style?.textAlignment ??
                                CrossAxisAlignment.start,
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              if (style?.showTitle ?? true)
                                TextPlaceholder(
                                  ...
                                  child: Text(
                                    semantic?.title?.clipAt(
                                            style?.maxTitleLetters ?? 22) ??
                                        "",
