...
/// [CollectionStreamControllers] is a class that provides access to the streams.
class CollectionStreamControllers {
  CollectionStreamControllers({
    this.manager,
  });
  final StreamController<Manager>? manager;
}

/// [AuthCollectionStreamControllers] is a class that provides access to the streams.
class AuthCollectionStreamControllers extends CollectionStreamControllers {
  AuthCollectionStreamControllers({
    super.manager,
  });
}

/// [CollectionStreams] is a class that provides access to the streams.
class CollectionStreams {
  CollectionStreams({
    this.manager,
  });
  final Stream<Manager>? manager;
}

/// [AuthCollectionStreams] is a class that provides access to the streams.
class AuthCollectionStreams extends CollectionStreams {
  final Stream<AuthModel>? model;
  final Stream<Token>? token;
  AuthCollectionStreams({
    super.manager,
    this.model,
    this.token,
  });
...
class ValidationException implements Exception {
  ...
  ValidationException({
    ...
    if (message?.isEmpty ?? true) {
      if (errors.length == 1) {
        ...
        this.message = '${errors.length} errors';
      }
    } else {
  ...
  ValidationError? find(String field) => errors.firstWhereOrNull(
...
abstract class Collection<T extends Model> {
  ...
  T singleModelFromMap(Map<String, dynamic> map);
  ...
  /// [copyWith] is used to copy the collection.
  Collection<T> copyWith({
    Manager? manager,
  });
...
abstract class AuthCollection<M extends Model, T extends AuthCredentials>
  ...
  Future<Response<Map<String, dynamic>>> signinR(
    ...
    try {
      ...
      if (e.response?.statusCode == 422 && e.response != null) {
        throw ValidationException.fromMap(e.response?.data);
      } else if (e.response?.statusCode == 400 && e.response != null) {
        throw ValidationException(
            message: "username or password is invalid",
            errors: [
              ValidationError(
                field: "username",
                message: "username may invalid",
                rule: '',
              ),
              ValidationError(
                field: "password",
                message: "password may invalid",
                rule: '',
              ),
            ]);
      } else {
...
/// [AuthResponse] is used to define the response of the auth request.
